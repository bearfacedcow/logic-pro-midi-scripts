...
function playChord(humanize, division, noteLength) {
  return activeNotes.forEach(note => {
    const humanizeVel =
...
function playArpeggio(humanize, division, noteLength, mode) {
  const humanizeVel =
    (Math.round(Math.random() * (humanize * 2)) - humanize) / 100;
  const humanizeBeat =
    ((Math.random() * (humanize * 2) - humanize) / 100) * division;

  if (mode == MODE_ARP_RND) {
    arpNextNote = Math.floor(Math.random() * activeNotes.length);
    if( arpNextNote == activeNotes.length ) {
      arpNextNote = activeNotes.length - 1;
    }
  } else {
    if( arpNextNote >= activeNotes.length ) {
      arpNextNote = activeNotes.length - 1;
  ...
  const note = activeNotes[arpNextNote];
  var noteOn = new NoteOn(note);
  noteOn.velocity += noteOn.velocity * humanizeVel;
  if (noteOn.velocity > 127) noteOn.velocity = 127;
    ...
    if (
      ...
      if (activeNotes.length > 0 && wasPlaying) {
        ...
        if (rhythm[nextStep] && schedule) {
          if (mode == MODE_CHORD) {
            playChord(humanize, division, noteLength);
          } else {
            playArpeggio(humanize, division, noteLength, mode);
          }
        }
      }
...
function ParameterChanged(param, value) {
  ...
  if (param == PARAMETER_MODE) {
    if (value == MODE_ARP_UP) {
      arpDirection = 1;
      arpPendulum = false;
    } else if (value == MODE_ARP_DN) {
      arpDirection = -1;
      arpPendulum = false;
    } else if (value == MODE_ARP_PEND) {
      arpDirection = 1;
      arpPendulum = true;
    }
  }
